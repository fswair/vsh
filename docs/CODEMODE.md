...
One MCP tool keeps discovery and prompt cost small without maintaining a parallel
command registry.
...
The ten functions use the Monty 0.0.22 integration. Each outer
...
Independent approval remains available through a trusted Python or Rust SDK host and is
intentionally absent from the model-facing MCP surface. The in-program `vsh_search`
function is part of `vsh_run.code`; it is not a separate MCP tool.
