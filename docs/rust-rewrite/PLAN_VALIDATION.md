...
|---|---|---|
| One semantic core for Rust and Python | Satisfied locally | `vsh-runtime` owns the facade; `vsh._native` is a thin PyO3 adapter. The shipped Python surface contains no simulator, policy, transaction state machine, or committer fallback. |
| Native crates.io SDK plus PyPI SDK | Satisfied locally | Eight publishable crate archives and one CPython 3.14 wheel/sdist build locally; the wheel installs in an empty environment and performs a bundled-worker preview and real commit. |
| Exact, reviewed, active dependencies | Satisfied for the supported target graph | Direct Rust dependencies are exact-pinned, `Cargo.lock`/`uv.lock` are frozen, 1,226 RustSec advisories were refreshed, `cargo-deny 0.20.2` passes advisories/bans/licenses/sources, and strict hash-locked `pip-audit 2.10.1` reports no vulnerability. The documented embedded-only `atomic-polyfill` lock entry is not selected by any supported target. |
...
- Local CPython 3.14 wheel, sdist, and eight crate archives: structure validation passes; isolated wheel preview/commit smoke passes.
