...
|---|---:|---|---|---|
| Rust | `1.95.0` | Rust project | compiler/toolchain | Matches Monty 0.0.21 MSRV. |
| `monty` | `=0.0.21` | Pydantic | interpreter | Stable published crate; MIT. |
| `monty-types` | `=0.0.21` | Pydantic | typed calls/limits | Same release train and MSRV. |
| `monty-proto` | `=0.0.21` | Pydantic | worker wire protocol and lossless Monty value → native Python conversion | Official same-release protocol/converter; worker feature disabled. |
| `monty-alloc` | `=0.0.21` | Pydantic | worker-wide bounded allocator | Official same-release allocator; only `exit-code`, defaults disabled. |
| `get-size2` | `=0.10.1` in `Cargo.lock` | get-size2 project | Monty/Ruff transitive memory accounting | Latest compatible patch from Monty 0.0.21's release lock; 0.10.3 changes `compact_str` to 0.10 and does not compile with Ruff 0.0.3's 0.9 type. |
...
1.15.0 and Monty 0.0.21 at check time. This is point-in-time evidence, not a
...
already made postcard part of the Monty 0.0.21 resolved graph. The direct VSH edge adds
...
The published `monty-runtime =0.0.21` binary and `monty-proto/worker` feature were
...
The current lockfile contains nine workspace packages and 223 registry package
...
`blake3 =1.8.7`, `cap-std =4.0.3`, and `postcard =1.1.3`; the rest are exact lockfile
resolutions of those dependencies.
Checks on 2026-08-29 refreshed 1,226 RustSec advisories and scanned all 232 lockfile
...
0.0.21 and its Ruff/parser/proc-macro graph (`getrandom 0.2.17`, `hashbrown 0.16.1`,
